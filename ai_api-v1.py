...
class LLMClient:
    ...
    async def chat(
        ...
        if not self.available:
            raise HTTPException(500, 'LLM backend not configured (set LLM_API_BASE and LLM_API_KEY)')
        body: Dict[str, Any] = {
            'model': self.model,
            'messages': messages,
...
SYSTEM_PROMPT = """You are ResearchGPT, an expert AI research assistant powered by a multi-engine search system.

You have access to the following tools:
- **search**: Search the web across multiple search engines (Google, Bing, Yahoo, DuckDuckGo, Yandex, etc.)
- **parse_url**: Extract clean markdown content from a single URL
- **parse_urls**: Parse multiple URLs into markdown in one call
- **gather_images**: Find and download relevant images
- **deep_research**: Run a full research pipeline (search → parse → summarize)

## Guidelines
1. When asked to research a topic, start with `search` to find relevant sources.
2. For important results, use `parse_url` to get full content before answering.
3. Use `deep_research` for comprehensive research tasks that need a summary.
4. Always cite your sources with URLs when providing information from searches.
5. Be concise but thorough. Prioritize accuracy over speed.
6. If a query is vague, search broadly first, then narrow down.
7. For academic research, mention that the `deep_research` tool supports `academic_only` mode.
"""
...
async def chat_completions(
    ...
    When the LLM requests a tool call, this endpoint executes the tool,
    feeds the result back, and returns the final answer.
    """
    # Extract session from first message or generate one
    session_id = None
